...
func requireProxmoxEnv(t *testing.T) proxmoxEnv {
	...
	if env.Host == "" || env.Node == "" || env.Storage == "" || env.VMID == "" || env.SourceImage == "" {
		t.Skip("set ELEMENTAL_PROXMOX_HOST, ELEMENTAL_PROXMOX_NODE, ELEMENTAL_PROXMOX_STORAGE, ELEMENTAL_PROXMOX_VMID, and ELEMENTAL_PROXMOX_SOURCE_IMAGE to run Proxmox integration tests")
	}
