...
func (i *ISO) Customize(d *deployment.Deployment) (err error) {
	...
	m[assetsPath] = filepath.Join("/", installDir)
...
func (i ISO) addInstallationAssets(root string, d *deployment.Deployment) error {
	...
	// copy deployment into a new instance, so we can modify the deployment
	// before writing it to the target root
	data, err := yaml.Marshal(d)
	if err != nil {
		return fmt.Errorf("failed marshalling deployment: %w", err)
	}
	d = &deployment.Deployment{}
	err = yaml.Unmarshal(data, d)
	if err != nil {
		return fmt.Errorf("failed unmarshalling deployment: %w", err)
	}
