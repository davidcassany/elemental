...
func (i Installer) Install(d *deployment.Deployment) (err error) {
	...
	err = i.addRecoveryBootEntry(cleanup, d)
	if err != nil {
		return fmt.Errorf("adding recovery boot entry")
	}

	return nil
}

func (i Installer) addRecoveryBootEntry(cleanup *cleanstack.CleanStack, d *deployment.Deployment) error {
	if d.GetRecoveryPartition() == nil {
		return nil
	}
	esp := d.GetEfiPartition()
	if esp == nil {
		return fmt.Errorf("no ESP partition defined in deployment")
	}

	espDir, err := vfs.TempDir(i.s.FS(), "", "elemental_esp")
	if err != nil {
		return err
	}
	cleanup.Push(func() error { return i.s.FS().RemoveAll(espDir) })

	device, err := block.GetPartitionDeviceByLabel(i.s, lsblk.NewLsDevice(i.s), esp.Label, 4)
	if err != nil {
		return fmt.Errorf("partition with label '%s', not found: %w", esp.Label, err)
	}

	err = i.s.Mounter().Mount(device, espDir, "auto", []string{"rw"})
	if err != nil {
		return fmt.Errorf("failed mounting ESP to %s: %w", espDir, err)
	}
	cleanup.Push(func() error { return i.s.Mounter().Unmount(espDir) })

	cmdline := fmt.Sprintf("%s %s", d.RecoveryKernelCmdline(), d.Installer.KernelCmdline)
	return i.b.AddBootEntry(espDir, bootloader.DefaultBootID, bootloader.RecoveryBootID, cmdline)
}
