...
With static configuration, node roles are assigned at **build time** based on hostname mappings in `kubernetes/cluster.yaml`:
...
No `nodes:` list is required in `kubernetes/cluster.yaml`. The token and role are passed via cloud instance user data.
...
├── release.yaml      # Product/release selection
├── butane.yaml       # System configuration (users, etc.)
├── userdata.yaml     # Enable dynamic configuration
└── kubernetes/
    └── cluster.yaml  # Helm charts and manifests (no nodes: list)
```
